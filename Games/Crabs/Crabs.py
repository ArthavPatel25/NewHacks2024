...
# Set up the display in fullscreen mode and get dimensions
...
# Load card images and background
card_images = {}
...
def draw_game(screen, game, message, cards=None, roll_value=None):
    ...
    if cards:
        card_width, card_height = 150, 210  # Increased size
        total_width = card_width * len(cards)
        start_x = (WIDTH - total_width) // 2
        for i, card in enumerate(cards):
            if card in card_images:
                card_image = card_images[card]
                card_image = pygame.transform.scale(card_image, (card_width, card_height))
                screen.blit(card_image, (start_x + i * card_width, HEIGHT // 2 - card_height // 2))
            else:
                pygame.draw.rect(screen, WHITE,
                                 (start_x + i * card_width, HEIGHT // 2 - card_height // 2, card_width, card_height))
                card_text = font.render(card, True, BLACK)
                screen.blit(card_text, (start_x + i * card_width + 10, HEIGHT // 2))
